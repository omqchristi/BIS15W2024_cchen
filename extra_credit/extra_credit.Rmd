...
```{r}
great_whites <- global_sharks %>%
  mutate(white_shark = str_detect(species, "white shark")) %>%
  mutate(white_shark2 = str_detect(species, "White shark")) %>%
  mutate(white_shark3 = str_detect(investigator_or_source, "Great White")) %>%
  filter(white_shark == TRUE | white_shark2 == TRUE | white_shark3 == TRUE)
great_whites %>%
  group_by(country) %>%
...
great_whites %>%
  ...
  summarise(total = n()) %>%
  pivot_wider(names_from = fatal_y_n, values_from = total)
```
...
How has types of incident rates changed over the years from 1900 - Current Day?

```{r}
global_sharks$year <- as.numeric(global_sharks$year)
```
...
global_sharks %>%
  ...
  geom_bar() +
  coord_flip() +
  labs(title = "Shark Incidents Over Time by Type",
       y = "Number of Incidents",
       x = "Year") +
  theme_classic()
```
