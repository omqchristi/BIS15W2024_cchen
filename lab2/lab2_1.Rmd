...
```{r}
z = 30 #do not use equal sign, can cause problems later on
```
...
```{r eval=FALSE, include=TRUE}
mean <- 20 #don't do this, could cause more problems
```
...
```{r}
mean(venom_GT, chiron, veyron) #this does not work, the number it outputs is not the arithmetic mean
...
The missing data messes up the mean calculation. R does not know what to do with the NA. It potentially messes up the calculations of any other code we decied to run.
...
Missing data is now removed, and R and can move on to doing a mean calculation with the rest of the data.
