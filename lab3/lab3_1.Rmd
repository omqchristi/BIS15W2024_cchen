...
```{r}
springs <- cbind(springs, depth_ft)
springs
```
...
```{r}
temp <- springs[,1] # you already have this information, when you made the data frame, so this line was unnecessary. only do this if you didn't make the matrix
mean(temp)
```
...
```{r}
write.csv(springs, "hsprings_data.csv", row.names=F)
```
