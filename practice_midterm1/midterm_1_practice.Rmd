...
```{r}
gabon %>%
  filter(Diversity_AllSpecies > 1.5) %>%
  group_by(LandUse) %>%
  summarise(mean_veg_rich = mean(Veg_Rich),
            mean_veg_stems = mean(Veg_Stems),
            mean_veg_liana = mean(Veg_liana),
            mean_veg_dbh = mean(Veg_DBH),
            mean_veg_canopy = mean(Veg_Canopy),
            total = n())
```
