...
```{r}
p + geom_point(size = 1.5, aes(color = thermoregulation))
```

We can also map shapes to another categorical variable.
```{r}
p + geom_point(size = 1.5, aes(color = thermoregulation, shape = thermoregulation))
```
...
homerange %>%
  ...
  geom_bar() + 
  coord_flip() +
  labs(title = "Count of Observations by Taxon",
        x = "Taxonomic Group",
        y = "Total Count")
```
...
penguins %>%
  ...
  geom_bar() +
  labs(title = "Penguin Population Across Islands",
       x = "Islands",
       y = "Total Population") +
