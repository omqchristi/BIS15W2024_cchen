...
life_history %>%
  ...
  ggplot(aes(y = order, x = mean_mass)) +
  geom_col()
```
...
life_history %>%
  group_by(order) %>%
  summarise(mean_mass = mean(mass, na.rm = T)) %>%
  ggplot(aes(y = reorder(order, mean_mass), x = mean_mass)) +
  geom_col() +
  scale_x_log10()
```
...
life_history %>%
  ggplot(aes(x = gestation, y = wean_mass)) +
  geom_jitter(na.rm = T) # this doesn't work for this data, but it usually prevents overplotting
```
