...
amniota %>%
  miss_var_summary() %>%
  summarize(sum = sum(n_miss)) # it looks like NA's are represented by -999
```
...
amphibio %>%
  miss_var_summary() %>%
  summarize(sum = sum(n_miss)) # normal NA use
```
...
```r
miss_var_summary(clean_amniota)
```
...
This does make sense. Mammals do not produce eggs, so they have a 100% NA rate for egg_mass_g. This means that some of the NAs are not missing data, they are purposefully left blank because of biology.
...
The NAs do not represent missing data. The NA represents the fact that the animal IS NOT that type. This means if 1104 species have NA for Ter, that means 1104 species are NOT terrestrial. This is simply an artifact of data collection.

**10. Now that we know how NA's are represented in the `amniota` data, how would you load the data such that the values which represent NA's are automatically converted?**

```r
cleaner_amniota <- read_csv("../data/amniota.csv", na = "-999")
```
