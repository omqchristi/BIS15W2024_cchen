...
ggplot(data = homerange,
       mapping = aes(y = log10.hra, x = log10.preymass)) +
  geom_point(na.rm = T) + #NAs are excluded from the plot by default
...
```{r}
mammals <- homerange %>% filter(taxon == "mammals")
```
...
mammals %>%
  arrange(log10.mass) %>%
  head(10) %>%
  ggplot(mapping = aes(x = log10.mass, y = common.name)) +
  geom_col()
```
