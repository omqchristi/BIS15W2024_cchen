...
```{r}
homerange %>%
  ggplot(aes(x = class, y = log10.mass, fill = class, group = taxon)) +
  geom_boxplot()
```
...
```{r}
homerange %>%
  ggplot(aes(x = class, y = log10.mass, fill = taxon, group = taxon)) +
  geom_boxplot()
```
...
deserts %>%
  ...
  summarise(observations = n(), .groups = 'keep') %>%
...
deserts %>%
  ...
  group_by(year, species_id) %>%
  summarise(observations = n(), .groups = 'keep') %>%
  ggplot(aes(x = year, y = observations, group = species_id, color = species_id)) +
  geom_line() +
  geom_point(shape = 8) +
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "lavenderblush4", fill = "lavenderblush2", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>%
  ...
  geom_histogram(fill = "lavender", color = "grey") +
  labs(title = "Distrubition of Homerange")
```
...
homerange %>%
  ggplot(aes(x = log10.hra,)) +
  geom_histogram(aes(y = after_stat(density)), fill = "lightblue", alpha = 0.5, color = "gray") +
  geom_density(color = "red") +
  labs(title = "Distrubition of Homerange")
```
...
```{r}
homerange %>%
  mutate(range_category = case_when(log10.mass <= 3.653 ~ "very small",
